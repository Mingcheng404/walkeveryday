  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      if (mode === 'signin') {
        ...
        // Step 1: Send OTP code to email (creates user if not exists)
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  if (error) throw error
                  setNoticeMessage('已重新寄送驗證碼到你的 Email。')
                } catch (e) {
                  setErrorMessage(e instanceof Error ? e.message : '重新寄送失敗。')
                }
              }}
              className="w-full text-center text-xs text-blue-300 underline underline-offset-2"
            >
              重新寄送驗證碼
            </button>
          )}
...
function normalizeAuthMessage(rawMessage: string, mode: AuthMode): string {
  ...
  if (m.includes('email not confirmed')) return '此帳號尚未驗證 Email，請先完成驗證。'
  ...
  if (m.includes('rate limit') || m.includes('over_email_send_rate_limit')) return '驗證碼寄送太頻繁，請稍後再試。'
  if (m.includes('email_address_invalid')) return 'Email 格式無效，請改用可接收郵件的真實地址。'
  if (m.includes('token') && (m.includes('invalid') || m.includes('expired'))) return '驗證碼無效或已過期，請重新寄送。'
