...
import { getRegionDistrictName, getRegionPolygons } from './districts'
import {
  destinationPoint,
  haversineKm,
  pointInMultiPolygon,
...
export async function generateRandomRoute(params: {
  ...
  estimatedTimeMins: number
  boundaries: DistrictFeatureCollection
}): Promise<GeneratedRoute> {
  const targetDistanceKm = Math.max(0.8, (params.estimatedTimeMins / 60) * AVERAGE_WALKING_SPEED_KMH)
  const polygons = getRegionPolygons(params.regionId, params.boundaries)

  let bestPath: LatLng[] = []
  let bestDistanceGap = Number.POSITIVE_INFINITY

  // Retry with different waypoint seeds and pick the closest one to target distance.
  for (let attempt = 0; attempt < 5; attempt += 1) {
    const loopWaypoints = buildLoopWaypoints(polygons, targetDistanceKm, params.estimatedTimeMins)
...
function buildLoopWaypoints(polygons: LngLat[][][], targetDistanceKm: number, minutes: number): LngLat[] {
  ...
  // Radius scales with target distance so short walks stay compact.
  ...
  for (let i = 0; i < waypointCount; i += 1) {
    ...
    if (!pointInMultiPolygon(candidate, polygons)) {
      const fallbackCandidate = randomPointInPolygons(polygons)
...
async function getRoutePath(waypoints: LngLat[]): Promise<LatLng[]> {
  try {
    ...
    // Intentionally fallback to synthetic path when public routing service fails.
  }
...
function buildFallbackPath(waypoints: LngLat[]): LatLng[] {
  ...
  if (waypoints.length > 0) {
    path.push(toLatLng(waypoints[waypoints.length - 1]))
  }
...
function dedupePath(path: LatLng[]): LatLng[] {
  ...
  for (let i = 1; i < path.length; i += 1) {
    ...
    if (haversineKm(last, current) > 0.003) {
      result.push(current)
    }
  }
